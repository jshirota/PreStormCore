...
namespace PreStormCore.Tools
{
    ...
    {
        public static IEnumerable<(string name, string code)> Generate(string url, string token, string user, string password, string @namespace, string domain)
        {
            ...
            {
                var tokenUrl = Regex.IsMatch(url, @"\.arcgis\.com/", RegexOptions.IgnoreCase)
...
{string.Join("\r\n", types.Select(x => $@"        public PreStormCore.{type}<{x.@class}> {x.property} {{ get; }}"))}
        public Service() : this(null, null, null, null) {{ }}
        public Service(string token) : this(null, null, null, token) {{ }}
        public Service(string userName, string password) : this(null, userName, password, null) {{ }}
        public Service(string url, string? userName, string? password) : this(url, userName, password, null) {{ }}
        private Service(string? url, string? userName, string? password, string? token)
        {{
            PreStormCore.{type}<T> Create<T>(int id) where T : PreStormCore.Feature => userName is null || password is null
                ...
                : new PreStormCore.FeatureLayer<T>($""{{url ?? defaultUrl}}/{{id}}"", userName, password);
